...
4. 모든 프롬프트에 대해 `hookSpecificOutput.additionalContext`를 주입한다.
5. classifier가 기술 구현/레퍼런스 작업으로 판단한 경우, Codex 본체가 주입된 컨텍스트를 읽고 실제 `web_search`를 수행한다.
6. no-web/local-only/trivial 작업은 no-web 결정 컨텍스트만 주입하고 검색은 하지 않는다.
7. 검색 근거를 짧게 남긴 뒤 개발/설계/수정으로 넘어간다.
...
- 검색으로 얻은 재사용 가능한 개발 지식을 JSON entry로 저장한다.
- 저장된 entry를 다시 JSON으로 검색해 다음 Codex 세션의 개인 지식 문서로 재사용한다.
- `context-hub`처럼 "검색으로 ID를 찾고 필요한 지식만 꺼내는" 사용감을 목표로 하되, v1은 외부 서비스 없이 로컬 JSON 파일만 사용한다.
...
- hook은 사용자의 프롬프트를 읽고 모든 프롬프트에 Knowledge-First 컨텍스트를 주입한다.
- classifier가 기술 구현/레퍼런스 작업으로 잡으면 Codex는 주입문을 보고 구현 전에 웹 검색한다.
- skill은 hook 주입문을 따르는 보조 행동 계약으로 포함한다.
- Knowledge Wiki CLI는 검색 지식을 JSON으로 저장하고 JSON으로 검색할 수 있다.
...
hook은 모든 프롬프트에 대해 Knowledge-First 컨텍스트를 주입한다. 그 다음 classifier가 웹 검색이 필요한지 판단한다.
...
3. **Decision Check**
   - 요청이 no-web/local-only/trivial 작업인지, 아니면 기술 구현/레퍼런스 작업인지 판단한다.
4. **Build Context**
   - 모든 프롬프트에 대해 Knowledge-First 컨텍스트를 만든다.
   - 기술 구현/레퍼런스 작업이면 요청 안의 기술적 검색 의도를 검색 지시로 만든다.
...
6. **Codex Searches When Required**
   - Codex는 classifier가 검색을 요구한 경우에만 파일 수정, 설계 확정, 의존성 추가 전에 주입문에 따라 웹 검색한다.
7. **Proceed**
   - 검색 근거를 1-3줄로 요약한 뒤 원래 작업을 진행한다.
   - 재사용 가능한 개발 지식이면 `scripts/knowledge-wiki.mjs`로 개인 Wiki에 저장한다.

## Knowledge Wiki

Search-First는 훅이 검색을 강제하는 것에서 끝나지 않고, 반복해서 쓰이는 개발 지식을 개인 Wiki로 누적할 수 있어야 한다.
...
테스트나 프로젝트별 실험에서는 `SEARCH_FIRST_WIKI_PATH`로 저장 위치를 바꿀 수 있다.
...
```bash
node scripts/knowledge-wiki.mjs add --title "React modal accessibility" --summary "Use ARIA dialog semantics." --tag react,a11y
...
v1 uses dependency-free lexical scoring. Title matches weigh highest, tag matches weigh next, and query/summary/content/source matches weigh lower. Entries use a stable ID derived from title/query when available, so re-adding the same search knowledge updates the existing JSON entry instead of creating duplicates.
...
hook은 항상 JSON stdout으로 `hookSpecificOutput.additionalContext`를 반환한다:
...
검색이 필요 없는 경우에도 `{}` 대신 no-web 결정이 담긴 Knowledge-First 컨텍스트를 반환한다.
...
- 검색이 필요한 이유
- 요청 안의 모든 기술적 검색 의도
- 파일 수정/구현 전에 검색하라는 지시 또는 no-web 결정
- 검색 근거를 1-3줄로 요약하라는 지시
- 재사용 가능한 개발 지식은 개인 JSON Wiki에 저장하라는 지시
...
- hook always returns `hookSpecificOutput.additionalContext`.
- hook returns a no-web decision context for no-web/local-only/trivial tasks.
- hook returns web-search instructions for classifier-matched development/technical tasks.
